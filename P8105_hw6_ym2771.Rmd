...
summary(mlr_all)
```
Firstly, I put all variables in the model, and based on the coefficient significance test, we choose variables with p-value of coefficient smaller than 0.005. Based on the overall regression, choosing the significant predictors to build a model, which includes `babysex`,`bhead`,`blength`,`delwt`,`gaweeks`,`mrace`,`smoke`.
...
As shown in plot, the residual and fitted value has no clear linear relationship, which means that the model 
...
### comment
Based on the plot above, the distribution of the 
